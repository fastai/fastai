...
def test_vision_pil2tensor():
    path  = URLs().LOCAL_PATH / "data/images"
    ...
    print(f"\nPIL.Image read {len(pil_passed)} of total {len(files)} images")
    print("\nPIL.Image read images with the following extensions:", 
           np.unique( [f.suffix for f in pil_passed] ))
    if len(pil_failed)>0: print("\nPIL.Image failed to these read images files:\n", np.asarray(pil_failed) )
    ...
    print(f"\npil2tensor read {len(pil2tensor_passed)} of total {len(pil_passed)}")
...
def test_vision_pil2tensor_16bit():
    f    = URLs().LOCAL_PATH / "data/images" / "gray_16bit.png"
