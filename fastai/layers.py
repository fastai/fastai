...
# %% ../nbs/01_layers.ipynb #79c7c159
@module()
def Identity(self, x):
    "Do nothing at all"
    return x

# %% ../nbs/01_layers.ipynb #09534c79
@module('func')
def Lambda(self, x):
    "An easy way to create a pytorch layer for a simple `func`"
    return self.func(x)

# %% ../nbs/01_layers.ipynb #ae0cfe0d
...
# %% ../nbs/01_layers.ipynb #9e5918a5
def sigmoid_range(x, low, high):
    "Sigmoid function with range `(low, high)`"
    return torch.sigmoid(x) * (high - low) + low

# %% ../nbs/01_layers.ipynb #5f7e3b7f
@module('low','high')
def SigmoidRange(self, x):
    "Sigmoid module with range `(low, high)`"
    return sigmoid_range(x, self.low, self.high)

# %% ../nbs/01_layers.ipynb #8a82a022
...
# %% ../nbs/01_layers.ipynb #c0a58fda
class PoolType: Avg,Max,Cat = 'Avg','Max','Cat'

# %% ../nbs/01_layers.ipynb #afc2e401
...
# %% ../nbs/01_layers.ipynb #7884a47e
NormType = Enum('NormType', 'Batch BatchZero Weight Spectral Instance InstanceZero')

# %% ../nbs/01_layers.ipynb #a25cb9b2
...
# %% ../nbs/01_layers.ipynb #bde81c49
def _conv_func(ndim=2, transpose=False):
    "Return the proper conv `ndim` function, potentially `transposed`."
    assert 1 <= ndim <=3
    return getattr(nn, f'Conv{"Transpose" if transpose else ""}{ndim}d')

# %% ../nbs/01_layers.ipynb #9c7dc00c
defaults.activation=nn.ReLU

# %% ../nbs/01_layers.ipynb #c090951a
...
# %% ../nbs/01_layers.ipynb #3b84dc4b
def AdaptiveAvgPool(sz=1, ndim=2):
    "nn.AdaptiveAvgPool layer for `ndim`"
    assert 1 <= ndim <= 3
    return getattr(nn, f"AdaptiveAvgPool{ndim}d")(sz)

# %% ../nbs/01_layers.ipynb #19cce66a
...
# %% ../nbs/01_layers.ipynb #46e2b2f0
def AvgPool(ks=2, stride=None, padding=0, ndim=2, ceil_mode=False):
    "nn.AvgPool layer for `ndim`"
    assert 1 <= ndim <= 3
    return getattr(nn, f"AvgPool{ndim}d")(ks, stride=stride, padding=padding, ceil_mode=ceil_mode)

# %% ../nbs/01_layers.ipynb #5fc6f6cb
def trunc_normal_(x, mean=0., std=1.):
    "Truncated normal initialization (approximation)"
    # From https://discuss.pytorch.org/t/implementing-truncated-normal-initializer/4778/12
    return x.normal_().fmod_(2).mul_(std).add_(mean)

# %% ../nbs/01_layers.ipynb #ecc0772f
...
# %% ../nbs/01_layers.ipynb #42ee64e9
class MergeLayer(Module):
    "Merge a shortcut with the result of the module by adding them or concatenating them if `dense=True`."
    def __init__(self, dense:bool=False): self.dense=dense
    def forward(self, x): return torch.cat([x,x.orig], dim=1) if self.dense else (x+x.orig)

# %% ../nbs/01_layers.ipynb #8308b4d5
...
# %% ../nbs/01_layers.ipynb #275f90da
inplace_relu = partial(nn.ReLU, inplace=True)

# %% ../nbs/01_layers.ipynb #191fa47e
...
# %% ../nbs/01_layers.ipynb #fff4901e
from torch.jit import script

# %% ../nbs/01_layers.ipynb #e5048c49
@script
