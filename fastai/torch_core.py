...
# %% ../nbs/00_torch_core.ipynb #2af84ffe
...
# %% ../nbs/00_torch_core.ipynb #b7e55e9e
_all_ = ['progress_bar','master_bar']

# %% ../nbs/00_torch_core.ipynb #e4d91e34
defaults.benchmark = True

# %% ../nbs/00_torch_core.ipynb #9977212d
...
# %% ../nbs/00_torch_core.ipynb #4bc9e4ef
...
# %% ../nbs/00_torch_core.ipynb #cbb8f8f7
def _fig_bounds(x):
    r = x//32
    return min(5, max(1,r))

# %% ../nbs/00_torch_core.ipynb #0cee112d
...
# %% ../nbs/00_torch_core.ipynb #e0ab2f70
@delegates(show_image, keep=True)
def show_titled_image(o, **kwargs):
    "Call `show_image` destructuring `o` to `(img,title)`"
    show_image(o[0], title=str(o[1]), **kwargs)

# %% ../nbs/00_torch_core.ipynb #667da15f
@delegates(subplots)
...
# %% ../nbs/00_torch_core.ipynb #a6701d40
...
# %% ../nbs/00_torch_core.ipynb #9d534f5c
...
# %% ../nbs/00_torch_core.ipynb #0cf105f6
...
# %% ../nbs/00_torch_core.ipynb #0ec24d19
...
# %% ../nbs/00_torch_core.ipynb #7da6c259
@contextmanager
...
# %% ../nbs/00_torch_core.ipynb #cf6fb70e
def unsqueeze(x, dim=-1, n=1):
    "Same as `torch.unsqueeze` but can add `n` dims"
    for _ in range(n): x = x.unsqueeze(dim)
    return x

# %% ../nbs/00_torch_core.ipynb #e57a66b0
def unsqueeze_(x, dim=-1, n=1):
    "Same as `torch.unsqueeze_` but can add `n` dims"
    for _ in range(n): x.unsqueeze_(dim)
    return x

# %% ../nbs/00_torch_core.ipynb #0a8e4438
def _fa_rebuild_tensor (cls, *args, **kwargs): return cls(torch._utils._rebuild_tensor_v2(*args, **kwargs))
def _fa_rebuild_qtensor(cls, *args, **kwargs): return cls(torch._utils._rebuild_qtensor  (*args, **kwargs))

# %% ../nbs/00_torch_core.ipynb #62359361
...
# %% ../nbs/00_torch_core.ipynb #0add56ac
...
# %% ../nbs/00_torch_core.ipynb #9c6a0718
...
# %% ../nbs/00_torch_core.ipynb #8c49c9cb
...
# %% ../nbs/00_torch_core.ipynb #cf02f7ba
def to_cpu(b):
    "Recursively map tensors in `b ` to the cpu."
    return to_device(b,'cpu')

# %% ../nbs/00_torch_core.ipynb #64919247
def to_np(x):
    "Convert a tensor to a numpy array."
    return apply(lambda o: o.data.cpu().numpy(), x)

# %% ../nbs/00_torch_core.ipynb #5e596ad5
...
# %% ../nbs/00_torch_core.ipynb #43afc2a6
...
# %% ../nbs/00_torch_core.ipynb #b0102326
@patch
...
# %% ../nbs/00_torch_core.ipynb #7bf6618d
...
# %% ../nbs/00_torch_core.ipynb #f3a89bfb
class TensorImageBase(TensorBase):
    _show_args = ArrayImageBase._show_args
    def show(self, ctx=None, **kwargs):
        return show_image(self, ctx=ctx, **{**self._show_args, **kwargs})

# %% ../nbs/00_torch_core.ipynb #2c7e8ea1
class TensorImage(TensorImageBase): pass

# %% ../nbs/00_torch_core.ipynb #21ad9109
class TensorImageBW(TensorImage): _show_args = ArrayImageBW._show_args

# %% ../nbs/00_torch_core.ipynb #456536fc
...
# %% ../nbs/00_torch_core.ipynb #ac1bfd04
...
# %% ../nbs/00_torch_core.ipynb #2611f9af
def concat(*ls):
...
# %% ../nbs/00_torch_core.ipynb #8d387f0c
class Chunks:
...
# %% ../nbs/00_torch_core.ipynb #051a7d1c
...
# %% ../nbs/00_torch_core.ipynb #22105ba5
class ShowTitle:
...
# %% ../nbs/00_torch_core.ipynb #6ea35e6d
@patch
...
# %% ../nbs/00_torch_core.ipynb #b31c7296
def get_empty_df(n):
    "Return `n` empty rows of a dataframe"
    df = pd.DataFrame(index = range(n))
    return [df.iloc[i] for i in range(n)]

# %% ../nbs/00_torch_core.ipynb #cf61cb82
def display_df(df):
...
# %% ../nbs/00_torch_core.ipynb #62eb74cb
def get_first(c):
    "Get the first element of c, even if c is a dataframe"
    return getattr(c, 'iloc', c)[0]

# %% ../nbs/00_torch_core.ipynb #e40d87d1
def one_param(m):
    "First parameter in `m`"
    return first(m.parameters())

# %% ../nbs/00_torch_core.ipynb #8de39371
...
# %% ../nbs/00_torch_core.ipynb #7f0f2a1d
def find_device(b):
    "Recursively search the device of `b`."
    return item_find(b).device

# %% ../nbs/00_torch_core.ipynb #20d61fa5
def find_bs(b):
...
# %% ../nbs/00_torch_core.ipynb #3cfb20d6
def np_func(f):
...
# %% ../nbs/00_torch_core.ipynb #7841871a
class Module(nn.Module, metaclass=PrePostInitMeta):
    "Same as `nn.Module`, but no need for subclasses to call `super().__init__`"
    def __pre_init__(self, *args, **kwargs): super().__init__()
    def __init__(self): pass

# %% ../nbs/00_torch_core.ipynb #fc954ec4
from torch.nn.parallel import DistributedDataParallel

# %% ../nbs/00_torch_core.ipynb #c05f5f83
def get_model(model):
    "Return the model maybe wrapped inside `model`."
    return model.module if isinstance(model, (DistributedDataParallel, nn.DataParallel)) else model

# %% ../nbs/00_torch_core.ipynb #828f7ed7
def one_hot(x, c):
...
# %% ../nbs/00_torch_core.ipynb #e1995342
def one_hot_decode(x, vocab=None):
    return L(vocab[i] if vocab else i for i,x_ in enumerate(x) if x_==1)

# %% ../nbs/00_torch_core.ipynb #bf0fc971
def params(m):
    "Return all parameters of `m`"
    return [p for p in m.parameters()]

# %% ../nbs/00_torch_core.ipynb #d508401f
def trainable_params(m):
    "Return all trainable parameters of `m`"
    return [p for p in m.parameters() if p.requires_grad]

# %% ../nbs/00_torch_core.ipynb #a8dc6e36
norm_types = (nn.BatchNorm1d, nn.BatchNorm2d, nn.BatchNorm3d, nn.InstanceNorm1d, nn.InstanceNorm2d, nn.InstanceNorm3d, nn.LayerNorm)

# %% ../nbs/00_torch_core.ipynb #ece21b66
...
# %% ../nbs/00_torch_core.ipynb #24847719
...
# %% ../nbs/00_torch_core.ipynb #057aed86
@patch
...
# %% ../nbs/00_torch_core.ipynb #645a6fbc
@patch
...
# %% ../nbs/00_torch_core.ipynb #432d9a53
def logit(x):
    "Logit of `x`, clamped to avoid inf."
    x = x.clamp(1e-7, 1-1e-7)
    return -(1/x-1).log()

# %% ../nbs/00_torch_core.ipynb #1db98603
def num_distrib():
    "Return the number of processes in distributed training (if applicable)."
    return int(os.environ.get('WORLD_SIZE', 0))

# %% ../nbs/00_torch_core.ipynb #77bd5034
def rank_distrib():
    "Return the distributed rank of this process (if applicable)."
    return int(os.environ.get('RANK', 0))

# %% ../nbs/00_torch_core.ipynb #9ae5bf02
def distrib_barrier():
    "Place a synchronization barrier in distributed training"
    if num_distrib() > 1 and torch.distributed.is_initialized(): torch.distributed.barrier()

# %% ../nbs/00_torch_core.ipynb #2bb2bd5d
# Saving arrays requires pytables - optional dependency
try: import tables
except: pass

# %% ../nbs/00_torch_core.ipynb #7fa177ce
def _comp_filter(lib='lz4',lvl=3): return tables.Filters(complib=f'blosc:{lib}', complevel=lvl)

# %% ../nbs/00_torch_core.ipynb #75f983a6
@patch
...
# %% ../nbs/00_torch_core.ipynb #5cdabce9
def doc(elt):
...
# %% ../nbs/00_torch_core.ipynb #d8891be8
...
# %% ../nbs/00_torch_core.ipynb #6e038106
...
# %% ../nbs/00_torch_core.ipynb #42bfc54f
...
# %% ../nbs/00_torch_core.ipynb #d8cc5f36
...
# %% ../nbs/00_torch_core.ipynb #7177a97b
def requires_grad(m):
    "Check if the first parameter of `m` requires grad or not"
    ps = list(m.parameters())
    return ps[0].requires_grad if len(ps)>0 else False

# %% ../nbs/00_torch_core.ipynb #6b9dee4f
...
# %% ../nbs/00_torch_core.ipynb #0af7f972
def cond_init(m, func):
    "Apply `init_default` to `m` unless it's a batchnorm module"
    if (not isinstance(m, norm_types)) and requires_grad(m): init_default(m, func)

# %% ../nbs/00_torch_core.ipynb #9d51dcee
def apply_leaf(m, f):
...
# %% ../nbs/00_torch_core.ipynb #66a3a4c3
def apply_init(m, func=nn.init.kaiming_normal_):
    "Initialize all non-batchnorm layers of `m` with `func`."
    apply_leaf(m, partial(cond_init, func=func))

# %% ../nbs/00_torch_core.ipynb #31c1ddba
...
# %% ../nbs/00_torch_core.ipynb #f4415e49
...
# %% ../nbs/00_torch_core.ipynb #c16553e7
...
# %% ../nbs/00_torch_core.ipynb #81098844
...
# %% ../nbs/00_torch_core.ipynb #b2f3d339
