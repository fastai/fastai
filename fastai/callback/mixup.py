...
__all__ = ['reduce_loss', 'mixup_criterion', 'MixHandler', 'MixUp', 'CutMix']
...
# Cell
def mixup_criterion(cb, lf, pred, yb):
    "A loss function for `MixUp` and `CutMix` style training"
    loss = torch.lerp(lf(pred, *cb.yb1), lf(pred, *yb), cb.lam)
    return reduce_loss(loss, getattr(cb.old_lf, 'reduction', 'mean'))

# Cell
class MixHandler(Callback):
    "A handler class for implementing `MixUp` style scheduling"
    run_after,run_valid = [Normalize],False
    def __init__(self, alpha=0.5, loss_func = mixup_criterion):
