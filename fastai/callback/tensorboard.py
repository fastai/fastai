...
__all__ = ['TensorBoardCallback', 'TensorBoardProjectorCallback', 'write_projector_embedding', 'add_projector_features']
...
class TensorBoardCallback(Callback):
    ...
    def __init__(self, log_dir=None, trace_model=True, log_preds=True, n_preds=9, projector=False, layer=None):
        store_attr('log_dir,trace_model,log_preds,n_preds,projector,layer')
    ...
    def after_pred(self):
        if self.run_projector:
            self.feat = add_projector_features(self.learn, self.h, self.feat)

    def after_validate(self):
        if self.run_projector:
            self.run_projector = False
            self._remove()
            write_projector_embedding(self.learn, self.writer, self.feat)
...
# Cell
def write_projector_embedding(learn, writer, feat):
    lbls = [learn.dl.vocab[l] for l in feat['lbl']] if getattr(learn.dl, 'vocab', None) else None

    writer.add_embedding(feat['vec'],
                metadata=lbls,
                label_img=feat['img'],
                global_step=learn.train_iter
                )

# Cell
def add_projector_features(learn, hook, feat):
