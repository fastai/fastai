...
# %% auto #0
__all__ = ['download_images', 'resize_to', 'verify_image', 'verify_images', 'resize_image', 'resize_images', 'search_images_bing',
           'search_images_google', 'search_images_yahoo', 'search_images_yandex', 'search_images_brave']
...
def _search_images_serpapi(params):
    "Takes `params` built for a specific engine and returns results from a SerpApi scrape."
    try:
        response = requests.get("https://serpapi.com/search", params=params)
        response.raise_for_status()
    except requests.HTTPError as e:
        warnings.warn(f"SerpApi request failed — ({e})", UserWarning, stacklevel=2)
        return L()
    return L(o['original'] for o in response.json()['images_results'] if 'original' in o)

def search_images_bing(key, term, max_images=150, scraper=_search_images_serpapi):
    "Search for `term` using `scraper` Bing Images engine scraper, returning up to `max_images` results."
    params = {
        'engine':'bing_images', 'q':term, 'count':max_images, 'photo':'photo', 'api_key':key
    }
    return scraper(params)

def search_images_google(key, term, scraper=_search_images_serpapi):
    "Search for `term` using `scraper` Google Images engine scraper, returning ~100 results."
    params = {
        'engine':'google_images', 'q':term, 'image_type':'photo', 'api_key':key
    }
    return scraper(params)

def search_images_yahoo(key, term, scraper=_search_images_serpapi):
    "Search for `term` using `scraper` Yahoo Images engine scraper, returning ~60 results."
    params = {
        'engine':'yahoo_images', 'p':term, 'imgty':'photo', 'api_key':key
    }
    return scraper(params)

def search_images_yandex(key, term, scraper=_search_images_serpapi):
    "Search for `term` using `scraper` Yandex Images engine scraper, returning ~30 results."
    params = {
        'engine':'yandex_images', 'text':term, 'file_type':'jpg', 'image_type':'photo', 'api_key':key
    }
    return scraper(params)
