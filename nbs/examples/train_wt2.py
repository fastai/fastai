...
def main(gpu:Param("GPU to run on", int)=6,
    ...
    if qrnn: cb_funcs = [partial(MixedPrecision, clip=0.1), ModelReseter, partial(RNNRegularizer, alpha=2, beta=1)]
    else : cb_funcs = [partial(MixedPrecision, clip=0.1), ModelReseter, partial(RNNRegularizer, alpha=3, beta=2)]
