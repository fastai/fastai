...
class DataBunch():
    ...
    def __init__(self, train_dl:DataLoader, valid_dl:DataLoader, fix_dl:DataLoader=None, test_dl:Optional[DataLoader]=None,
        ...
        self.train_dl,self.valid_dl = map(_create_dl, [train_dl,valid_dl])
        self.fix_dl = train_dl.new(suffle=False, drop_last=False) if fix_dl is None else _create_dl(fix_dl)
    ...
    def _init_ds(train_ds:Dataset, valid_ds:Dataset, test_ds:Optional[Dataset]=None):
        try:    fix_ds = valid_ds.new(train_ds.x, train_ds.y) # train_ds, but without training tfms
        except: fix_ds = train_ds
